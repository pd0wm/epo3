...
begin
	...
	begin
		if (
			unsigned(pos_x_in) >= line_fp and unsigned(pos_x_in) < line_fp + line_pw and unsigned(pos_y_in) >= offset_y
		) then
			h_sync_new <= '1';
		else
			h_sync_new <= '0';
	...
	process(pos_x_in, pos_y_in)
	begin
		if (unsigned(pos_y_in) >= frame_fp and unsigned(pos_y_in) < frame_fp + frame_pw) then
			v_sync_new <= '1';
		else
			v_sync_new <= '0';
		end if;
